...
mod test {
    ...
    #[rstest]
    #[case(Ntsc::False(), false, false, false)]
    #[case(Ntsc::NonDropFrame(), true, true, false)]
    #[case(Ntsc::DropFrame(), true, false, true)]
    fn test_ntsc(
        #[case] value: Ntsc,
        #[case] check: bool,
        #[case] non_drop: bool,
        ...
        assert_eq!(check, value.check(), "check() is expected for {:?}", value);
        assert_eq!(
            non_drop,
            value.is_non_dropframe(),
            "is_non_dropframe() is expected for {:?}",
            value
        );
        assert_eq!(
            drop,
            value.is_dropframe(),
            "is_dropframe() is expected for {:?}",
            value
        );
    }
